...
for i in range (len(lnamefile)): # i contient l'indice du nom du fichier dans lnamefile
    ...
    ma_carte=folium.Map(location=[llatitude[i],llongitude[i]],zoom_start=4)
    folium.Marker(location=[llatitude[i],llongitude[i]],popup=label,icon=icone_marqueur).add_to(ma_carte)
